...
This record is written to the browser's extension-local storage, which is
private to this extension inside your browser profile. It is never uploaded
anywhere.

The extension does not use browser sync storage and does not send data to any
server.
...
- **Reset settings** — Settings → Reset returns all preferences to their
  defaults.
- **Uninstall Extensions Update Tracker** — the browser removes the extension's
  local storage along with it, deleting the whole update history.
